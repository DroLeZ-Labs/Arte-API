...
1. `Automated Request Validation`: Just tell the endpoint initializer what is your expected request structure in a descriptive manner and the request method, and the framework will validate it.
```php
...
### 4. Database Mappers
in app/model/mappers, you can define mappers for your system entities. Mappers correspond to your db tables in 1:1 correspondance and must be named as "<entity_name_here>Mapper". Mapper objects are coupled with Entity objects so that it can provide with db auto-sync as you are going to see later. Mapper base class provide you with the following:
...
```php
$entity_from_db->get('property1'); // automatically loads the object information from db if not loaded and that reduces code complexity and won't fire a db call until you actually need it.
```
...
The `Router::route()` method searches for the requested endpoint by the client and returns an instance of the `Endpoint` class if found. This allows us to call.
...
3. The Router then searches for the requested endpoint inside routes.json and specifies the endpoint class location with request params.
4. The decided endpoint is then run meaning that the request is validated and sanitized, then endpoint prehandles and handle are run.
5. The endpoint response buffer is then formatted and sent to the client.
